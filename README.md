...
- 클러스터링은 비지도 KMeans가 아니라, 5거래일 선행 수익률을 6개 구간으로 나눈 뒤 각 구간의 뉴스 피처 중심점을 저장하는 방식임.
...
5. `body_sentiment_decay_3d`는 마지막 실제 뉴스의 `body_sentiment_score`를 가져와 `0.5^(days_since_news / 3)`로 감쇠
...
- 오래된 뉴스의 잔존 영향은 `days_since_news`, `body_sentiment_decay_3d`, 감쇠된 `body_emb_*`로 따로 표현함
...
현재 방식은 뉴스 피처끼리만 비지도 군집화하는 KMeans가 아니라, 미래 수익률 레이블을 먼저 정하고 그 레이블별 뉴스 피처 중심점을 저장하는 방식임.
